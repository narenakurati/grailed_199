...
load_data <- function(){
  ...
  #create temp frame
  temp_frame <- setNames(data.frame(matrix(ncol = 6, nrow = 10)), c("title", "price", "brand", "size", "size", "time"))
  ...
  for (i in 1:80){
    temp_url <- sold_page %>% html_nodes(xpath = '//*[@id="homepage-v2"]/div/div/div[2]/div/div[2]/div/div[2]/div[1]/a') %>% xml_attr("href")
    temp_url <- paste("https://www.grailed.com", temp_url, sep = "")
    print(temp_url)
    temp_frame[1,] <- pull_and_store_internal(temp_url)
    print(temp_frame)
  }
}
